...
namespace ObjectiveExecutor {
    ...
    bool IsSeaOrHoverObjective(const Objectives::StrategicObjective@ objective)
    {
        ...
        for (uint r = 0; r < objective.roles.length(); ++r) {
            ...
            if (role == AiRole::SEA || role == AiRole::TACTICAL) return true;
        }
        return false;
    }
    ...
    string unitNameForCount = resolvedUnitName;
        if (btype == Task::BuildType::FACTORY && s.type == Objectives::BuildingType::SEAPLANE_FACTORY) {
            ...
            // Objective-driven T1 energy currently cannot associate a specific builder; pass -1 as builderId.
            // Builder-side logic treats negative ids as "no assist redirect" and will only enforce cooldown.
            @task = Builder::EnqueueT1Solar(-1, side, anchor, shake, 300 * SECOND, s.priority);
