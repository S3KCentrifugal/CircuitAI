...
namespace UnitHelpers {
    ...
    *************************************************************************************/
    ...
    // Combined list of all land lab unit names across factions, tiers (deduplicated)
    array<string> GetAllLandLabs() {
        array<string> result;
        dictionary seen;
    ...
    array<string> GetAllFastT2Bots() {
        return GetFastT2Bots("");
    }
    ...
    // Given a factory name and side, resolve an appropriate cheap T1 land raider unit
    // for the Front role. This is used for one-time early-game spam after constructors.
    // Conceptually this is the "raider class" (e.g., armpw/corak/leggob or their vehicle equivalents).
    string GetFrontT1RaiderForFactory(const string &in factoryName, const string &in side)
    {
        // For now, we reuse the same mapping as scouts/raiders, since those are the lightest
        // frontline units per side. This keeps the behavior consistent and centralized.
        if (IsT1BotLab(factoryName)) {
            return GetT1BotScoutForSide(side);
        }
        if (IsT1VehicleLab(factoryName)) {
    ...
    // Vehicle constructors (T2) across all sides
    array<string> GetAllT2VehicleConstructors() {
        array<string> ids;
        ids = { "armacv", "coracv", "legacv" };
        return ids;
    }

    // Side-specific T2 vehicle constructors (exclude bots)
    array<string> GetT2VehicleConstructors(const string &in side) {
        array<string> ids;
        if (side == "armada") {
            ids = { "armacv" };
        } else if (side == "cortex") {
            ...
            ids = GetAllT2VehicleConstructors();
        }
        return ids;
    }
    ...
    // Side-specific T2 bot labs
    array<string> GetT2BotLabs(const string &in side) {
        array<string> ids;
        if (side == "armada") {
            ids = { "armalab" };
        } else if (side == "cortex") {
    ...
    // Side-specific T2 vehicle labs
    array<string> GetT2VehicleLabs(const string &in side) {
        array<string> ids;
        if (side == "armada") {
            ids = { "armavp" };
        } else if (side == "cortex") {
    ...
    bool IsT1Mex(const string &in name) {
        return name == "armmex" || name == "cormex" || name == "legmex";
    }

    bool IsT2Mex(const string &in name) {
        return name == "armmoho" || name == "cormoho" || name == "legmoho";
    }
    ...
    array<string> GetAllT2AirConstructors()
    {
        array<string> ids = { "armaca", "coraca", "legaca" };
        return ids;
    }

    array<string> GetAllT1SeaConstructors()
    {
        array<string> ids = { "armcs", "corcs", "legnavyconship" };
        return ids;
    }
    ...
    {
        array<string> ids;
        { array<string> a = GetAllT2BotConstructors(); for(uint i=0;i<a.length();++i) ids.insertLast(a[i]); }
        { array<string> a = GetAllT2VehicleConstructors(); for(uint i=0;i<a.length();++i) ids.insertLast(a[i]); }
        { array<string> a = GetAllT2AirConstructors(); for(uint i=0;i<a.length();++i) ids.insertLast(a[i]); }
        { array<string> a = GetAllT2SeaConstructors(); for(uint i=0;i<a.length();++i) ids.insertLast(a[i]); }
        return ids;
    }
    ...
    // All T1 Bot Scouts
    array<string> GetAllT1BotScouts() {
        array<string> ids = { "armflea", "corak", "leggob" };
        return ids;
    }

    // All T1 Vehicle Scouts
    array<string> GetAllT1VehicleScouts() {
        array<string> ids = { "armflash", "corfav", "legscout" };
        return ids;
    }

    array<string> GetAllT1AircraftScouts()
    {
        array<string> ids;
    ...
    // Helper: return true if the given unit name is a T1 bot constructor (exclude vehicles/air/sea)
    bool IsT1BotConstructor(const string &in unitName)
    {
        array<string> ids = GetAllT1BotConstructors();
        for (uint i = 0; i < ids.length(); ++i) {
            if (ids[i] == unitName) return true;
        }
        return false;
    ...
    bool IsT2BotConstructor(const string &in unitName)
    {
        array<string> ids = GetAllT2BotConstructors();
        for (uint i = 0; i < ids.length(); ++i) {
            if (ids[i] == unitName) return true;
        }
        return false;
    ...
    // Helper: return true if the given unit name is in the fast-assist bot list
    bool IsFastAssistBot(const string &in unitName)
    {
        array<string> ids = GetAllFastAssistBots();
        for (uint i = 0; i < ids.length(); ++i) {
            if (ids[i] == unitName) return true;
        }
        return false;
    ...
    // Combined list of all experimental gantries (land + water)
    array<string> GetAllGantries() {
        array<string> ids; dictionary seen;
        { array<string> a = GetAllLandGantries(); _AppendUniqueNonEmpty(@a, @ids, @seen); }
        { array<string> a = GetAllWaterGantries(); _AppendUniqueNonEmpty(@a, @ids, @seen); }
        return ids;
    }

    // All T2 Metal Extractors (Moho Mines)
    array<string> GetAllT2Mexes() {
        array<string> ids = { "armmoho", "cormoho", "legmoho" };
        return ids;
    }

    // T2 Metal Extractor (Moho Mine) name per side
    string GetT2MexNameForSide(const string &in side)
    {
        if (side == "armada") return "armmoho";
        if (side == "cortex") return "cormoho";
        if (side == "legion") return "legmoho";
        return "armmoho";
    }

    // T1 Air Constructor name per side
    string GetT1AirConstructorNameForSide(const string &in side)
    {
        if (side == "armada") return "armca";
        if (side == "cortex") return "corca";
        if (side == "legion") return "legca";
        return "armca";
    }

    // T2 Air Constructor name per side
    string GetT2AirConstructorNameForSide(const string &in side)
    {
        if (side == "armada") return "armaca";
        if (side == "cortex") return "coraca";
        if (side == "legion") return "legaca";
        return "armaca";
    }

    // Amphibious T2 Bot name per side
    string GetAmphibiousT2BotForSide(const string &in side)
    {
        if (side == "armada") return "armamph"; // Platypus
        if (side == "cortex") return "coramph"; // Duck
        if (side == "legion") return "legamph"; // Telchine
        return "armamph";
    }
